(async () => {
  ...
  const DEFAULT_SEARCH_DATE_TYPE = 'PICKING_START_DATE';
  const DEFAULT_SELECTED_STATUSES = ['STOWED'];
  const DEFAULT_QUICK_RANGE = '7';
  const CONCURRENCY = 3;
  ...
  const PAGE_SIZE = 20;
  const MAX_PAGE = 200;
  const MAX_GC_PER_CT = 100;
  const STORAGE_KEY = 'grabCtPanelSettingsV2';
  const PANEL_ID = 'ct-location-grab-modal';
  const STYLE_ID = 'ct-location-grab-style';
...
`.trim().split(/\s+/);
  ...
  const createPanel = () => {
    ...
    const getOptions = () => {
      ...
      const toCenterCode = normalizeCenterCode(toCenterInput.value) || '';
