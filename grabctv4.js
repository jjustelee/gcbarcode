...
`.trim().split(/\s+/);
  ...
  const createPanel = () => {
    ...
    const initialQuickRange = settings.quickRange === undefined
      ? DEFAULT_QUICK_RANGE
      : String(settings.quickRange || '');
    const initialRange = initialQuickRange
      ? getQuickDateRange(initialQuickRange)
      : {
          startDate: settings.startDate || getQuickDateRange(DEFAULT_QUICK_RANGE).startDate,
          endDate: settings.endDate || getQuickDateRange(DEFAULT_QUICK_RANGE).endDate
        };
