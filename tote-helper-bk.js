(function () {
  ...
  function setKey(fn) {
    ...
    keyHandler = fn;
    if (fn) document.addEventListener("keydown", keyHandler, true);
  }
  ...
  style.textContent =
    ...
    "#" + APP_ID + " .qa{margin-top:24px;padding-top:18px;border-top:1px solid #ddd}" +
    "#" + APP_ID + " .qa button{font-size:18px;min-width:auto;padding:10px 14px;border-radius:10px;background:#999}";
  ...
  function loadBarcode(cb) {
    ...
    s.onerror = function () {
      ...
      if (el) el.textContent = "바코드 생성 실패: CDN 차단 가능";
    };
  ...
  function makeBarcode(value) {
    loadBarcode(function () {
      try {
        ...
        var el = document.getElementById("barcodeError");
        if (el) el.textContent = "바코드 생성 오류: " + e.message;
      }
    });
  }
  ...
  function showSelect(sku) {
    var rows = [row(sku, 1), row(sku, 2)];
    ...
    var html = '<h1 class="multiTitle">해당하는 상품 번호를 누르세요</h1><div class="multiList">';

    for (var i = 0; i < rows.length; i++) {
      html +=
        '<button class="card" data-i="' + i + '">' +
        '<div class="q">' + (i + 1) + "번 선택 · 수량 " + h(rows[i].quantity) + "</div>" +
        '<div class="p">' + h(rows[i].skuName) + "</div>" +
        '<div class="s">상품바코드 ' + h(rows[i].skuBarcode) + "</div>" +
        '<div class="t">토트 ' + h(rows[i].toteBarcode) + "</div>" +
        "</button>";
    }

    html += '</div><button id="backBtn" class="secondary smallBtn">다시 Esc</button>';
    ...
    for (var j = 0; j < items.length; j++) {
      items[j].onclick = function () {
        showConfirm(rows[Number(this.getAttribute("data-i"))]);
      };
    }
  }
  ...
  function showEmpty(sku) {
    ...
    render(
      '<h1>조회 없음</h1>' +
      '<div class="error">찾을 수 없음요. PS사원님께 문의해주세요~!!!</div>' +
      '<div class="sku">' + h(sku) + "</div>" +
