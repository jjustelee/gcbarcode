(function () {
  ...
  // 5. WMS 조회
  // ================================
  function buildInventoryUrl(sku) {
    return (
      CONFIG.wms.baseUrl +
      "?searched=true" +
      "&locationType=" +
  ...
  function loadBarcode(cb) {
    ...
    if (el) {
      el.textContent = "바코드 생성 실패: 확장 프로그램에 JsBarcode가 포함되어 있는지 확인하세요.";
    }
  }
  ...
  function showLoading(sku, next) {
    ...
    setTimeout(next, CONFIG.timing.loadingDelayMs);
  }
